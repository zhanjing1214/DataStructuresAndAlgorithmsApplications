...
public class Knapsack {
    private BigIntegerDoubleLinkedList privateKey;
    private BigIntegerDoubleLinkedList publicKey;
     ...
     */
    public Knapsack() {
        this.privateKey = new BigIntegerDoubleLinkedList();
        this.publicKey = new BigIntegerDoubleLinkedList();
        Scanner sc;
        String text;
        do{
    ...
    private void generateAllKeys() {
        ...
        for(int i = 1; i < 640; i ++) {
            BigIntegerDoubleNode endNode = privateKey.getEnd();
            sum = sum.add(endNode.getBigInt());
            BigInteger newBigInt = new BigInteger("" +randomInt.nextInt(8));
            privateKey.addBigIntAtEnd(newBigInt.add(sum));
        }
        //generate m
        sum = sum.add(privateKey.getEnd().getBigInt());
